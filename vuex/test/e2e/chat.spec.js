...
describe("e2e/chat", () => {
  ...
  test(
    ...
    async () => {
      ...
      expect(await text(".thread-list-item.active")).toContain(
        "Functional Heads"
      );
      expect(await text(".message-thread-heading")).toContain(
        "Functional Heads"
      );
      expect(await count(".message-list-item")).toBe(2);
      expect(
        await text(".message-list-item:nth-child(1) .message-author-name")
      ).toContain("Bill");
      expect(
        await text(".message-list-item:nth-child(1) .message-text")
      ).toContain("Hey Brian");
